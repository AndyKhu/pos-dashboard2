...
const UnitPage = () => {
  ...
  const EventHandler = {
    onSubmit: async (values: enumFormType) => {
      ...
      if(res.ok){
        toast({
          title: `UNIT ${values.id?"Updated":"Saved"}!`,
        ...
        if(values.id)
          ...
          setUnitLists(old => [...old, resData.data])
        form.reset()
      }else{
        toast({
          title: `Failed to ${values.id?"Update":"Save"} Unit!`,
  ...
  const [unitLists, setUnitLists] = useState<TEnumDB[]>([])
  const EventHandlerTable = {
    handleSelectedDelete: async ()=> {
      ...
      if(res.ok){
        ...
        setUnitLists(old => old.filter(i => !SelectedRow.includes(i.id||"")))
        form.reset()
      }else{
        toast({
    ...
    handleSelectedEdit: ()=> {
      ...
      if(SelectedRow.length > 1 ||  SelectedRow.length === 0){
        ...
        const selectedData = unitLists.find(item => item.id === SelectedRow[0])
  ...
  return (
    ...
    list={<EnumLists data={unitLists} headerOption={headerOption} selection={{rowSelection,setRowSelection}} eventHandler={EventHandlerTable} permission={menupermission}/>}
